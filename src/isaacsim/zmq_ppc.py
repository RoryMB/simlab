...
"""
zmq_ppc.py - ZMQ Pick and Place Client (Orchestrator)
...
class PF400Orchestrator:
    ...
    def __init__(self, port=5557, timeout=10000):
        ...
        logger.info(f"Connected to PF400 robot server on port {port}")
        ...
        while time.time() - start_time < max_wait:
            ...
            if response.get("status") == "success":
                if response.get("collision_detected", False):
                    logger.error("Collision detected during motion")
                    return False
                elif response.get("motion_complete", False) or not response.get("is_moving", False):
                    logger.info("Robot completed motion")
                    return True
                    
            time.sleep(check_interval)
            
        logger.warning(f"Robot did not complete motion within {max_wait} seconds")
        return False
    ...
    def move_to_pickup_position(self):
        ...
        logger.info("=== MOVING TO PICKUP POSITION ===")
    ...
    def move_to_dropoff_position(self):
        ...
        logger.info("=== MOVING TO DROPOFF POSITION ===")
    ...
    def pick_and_place_sequence(self):
        ...
        if not self.move_to_safe_height():
            logger.error("Failed to lift to safe height")
            # Continue anyway
            
        # Step 5: Move to dropoff position
        if not self.move_to_dropoff_position():
            logger.error("Failed to reach dropoff position")
            return False
    ...
    def collision_test_sequence(self):
        ...
        logger.info("Commanding robot to move to potential collision position...")
        collision_joints = [
        ...
        if response.get("status") != "success":
            logger.error("Failed to send collision test command")
            return False
        ...
        if status.get("collision_detected", False):
            logger.info("SUCCESS: Collision detected as expected!")
            return True
        else:
            logger.warning("No collision detected - may need to adjust test")
            return False

    def manual_joint_move(self, joints):
        """Move to specific joint positions"""
        if len(joints) != 7:
            logger.error("Need exactly 7 joint values")
            return False
    ...
    def interactive_mode(self):
        ...
        logger.info("  home - Move to home position")
        logger.info("  pickup - Move to pickup position") 
        logger.info("  dropoff - Move to dropoff position")
        logger.info("  safe - Move to safe height")
        logger.info("  close - Close gripper")
        logger.info("  open - Open gripper")
        logger.info("  status - Get robot status")
        logger.info("  sequence - Run full pick and place sequence")
        logger.info("  collision - Test collision detection")
        logger.info("  joints <j1> <j2> ... <j7> - Move to specific joint angles")
            ...
            try:
                ...
                if cmd[0] == "quit":
                    ...
                    try:
                        ...
                        logger.error("Invalid joint values - must be numbers")
                else:
                    logger.error(f"Unknown command or wrong number of arguments: {' '.join(cmd)}")
    ...
    try:
        if args.mode == "sequence":
            ...
            if success:
                ...
                logger.error("Pick and place sequence failed")
                
        elif args.mode == "collision":
            success = orchestrator.collision_test_sequence()
            if success:
                logger.info("Collision test completed successfully")
            else:
                logger.error("Collision test failed")

...
Robot nodes (in `core/robots/`) require MADSci server URLs from the `.env` file.
...
cd core/robots/ur5e && ./run_node_ur5e.sh
```

Or combine into a single command:
```bash
(set -a; source projects/my-project/madsci/config/.env; set +a; cd core/robots/ur5e && ./run_node_ur5e.sh)
```
